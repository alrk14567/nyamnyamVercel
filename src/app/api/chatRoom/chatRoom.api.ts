...
export async function insertChatRoom(chatRoom: ChatRoomModel): Promise<any | { status: number; data?: any; message?: string }> {
  try {
    const response = await fetch('https://kidonapi.nyamnyam.kr/api/chatRoom/save', {
      method: 'POST',
      headers: {
...
export async function checkChatRoom(chatRoom: ChatRoomModel): Promise<any | { status: number; data?: any; message?: string }> {
  try {
    const response = await fetch('https://kidonapi.nyamnyam.kr/api/chatRoom/check', {
      method: 'POST',
      headers: {
        'Authorization': token ? `Bearer ${token}` : '',
        "Content-Type": "application/json",
...
export const fetchChatRooms = async (nickname: any) => {
  console.log(token)
  const response = await fetch(`https://kidonapi.nyamnyam.kr/api/chatRoom/findAll/${nickname}`, {
    method: 'GET',
    headers: {
      'Authorization': token ? `Bearer ${token}` : '',
      "Content-Type": "application/json",
...
export const fetchChatRoomCount = async () => {
  ...
  const response = await fetch('https://kidonapi.nyamnyam.kr/api/chatRoom/count', {
    method: 'GET',
    headers: {
      'Authorization': token ? `Bearer ${token}` : '',
      "Content-Type": "application/json",
...
export const fetchChatRoomById = async (chatRoomId: any) => {
  ...
  const response = await fetch(`https://kidonapi.nyamnyam.kr/api/chatRoom/${chatRoomId}`, {
    method: 'GET',
    headers: {
      'Authorization': token ? `Bearer ${token}` : '',
      "Content-Type": "application/json",
...
export const deleteChatRoomApi = async (chatRoomId: string, nickname: string) => {
  const response = await fetch(`https://kidonapi.nyamnyam.kr/api/chatRoom/leaveChatRoom/${chatRoomId}/${nickname}`, {
    method: 'DELETE',
    headers: {
      'Authorization': token ? `Bearer ${token}` : '',
      "Content-Type": "application/json",
