...
export const subscribeToChats = (chatRoomId: any, onMessageReceived: (arg0: any) => void) => {
  const eventSource = new EventSourcePolyfill(`https://kidonapi.nyamnyam.kr/api/chats/${chatRoomId}`, {
    method: "GET",
    headers: {
...
export const sendChat = async (chatRoomId: any, chat: any) => {
  const response = await fetch(`https://kidonapi.nyamnyam.kr/api/chats/${chatRoomId}`, {
    method: "POST",
    headers: {
      'Authorization': token ? `Bearer ${token}` : '',
      "Content-Type": "application/json",
...
export const getUnreadCount = async (chatRoomId: string, nickname: string): Promise<number> => {
  const response = await fetch(`https://kidonapi.nyamnyam.kr/api/chats/${chatRoomId}/unreadCount/${nickname}`, {
    method: "GET",
    headers: {
      'Authorization': token ? `Bearer ${token}` : '',
      "Content-Type": "application/json",
...
export const getNotReadParticipantsCount = async (chatId: string): Promise<number> => {
  const response = await fetch(`https://kidonapi.nyamnyam.kr/api/chats/${chatId}/notReadParticipantsCount`, {
    method: "GET",
    headers: {
      'Authorization': token ? `Bearer ${token}` : '',
      "Content-Type": "application/json",
...
export const markMessageAsRead = async (chatId: string, nickname: string): Promise<any> => {
  const response = await fetch(`https://kidonapi.nyamnyam.kr/api/chats/${chatId}/read/${nickname}`, {
    method: 'PUT',
    headers: {
      'Authorization': token ? `Bearer ${token}` : '',
      "Content-Type": "application/json",
